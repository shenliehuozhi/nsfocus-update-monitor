...
- [ ] 🔴 **产品管理 Web UI**：自动发现 + 开关管理 + 手动注册（详见 系统架构.md §3.6）
  - 数据库迁移：content_sources 加 entry_url + strategy 列
  - nsfocus.py：PRODUCTS 硬编码 → `_get_products()` DB 动态读取
  - scheduler.py：全链路 PRODUCTS 引用替换
  - system_routes.py：`/api/products` CRUD + `POST /api/products/discover` + `POST /api/products/detect`
  - index.html：系统设置页 → 产品开关面板 + 重新扫描 + 手动注册弹窗
