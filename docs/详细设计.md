...
过滤维度:
1. chains:      snap_chain（前缀/精确）匹配 conditions['chains']
   - leaf:    snap_chain == rule_chain（精确到具体包类型）
   - subtree: snap_chain 以 rule_chain 为前缀（订阅该节点下全部）
   - chain 反查: 通过 snap.source_url 在 source.package_type.paths 中实时查找
2. urgency:     snap['urgency'] in conditions['urgency']
3. keywords:     any(kw in description_raw for kw in keywords)

空 conditions（{}）= 匹配全部（向后兼容）
```
...
snapshot.source_url
    ↓
source.package_type.paths[] → 找 p.url == snapshot.source_url
    ↓
p.chain  →  ["WAF", "标准正式版", "V6.0.8", "规则"]
```

运行时通过 `scheduler._get_chain(source_id, source_url)` 从内存缓存
`_url_chain_cache[source_id][norm_url] → chain` 中获取，无需 schema 改动。

**filter_conditions 结构**：
